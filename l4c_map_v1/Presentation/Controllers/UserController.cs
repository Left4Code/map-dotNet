    ...
    public class UserController : Controller
    {
        ...
        public async Task<ActionResult> Login(Credential uvm)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                HttpResponseMessage response = await client.PostAsJsonAsync("/l4c_map-v1-web/rest/authentication", uvm);
                Session["token"] = response.Content.ReadAsStringAsync().Result.ToString();
                HttpResponseMessage response1 = client.GetAsync("/l4c_map-v1-web/rest/user/" + uvm.username).Result;
